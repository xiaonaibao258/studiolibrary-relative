...
同一个 Relative Pose 文件会同时保存完整的原版 Pose 属性数据和相对矩阵。关闭 **Relative** 时，系统按原版方式加载完整属性（包括自定义属性）；开启 **Relative** 时，为避免意外覆盖动画开关、IK/FK、空间切换等数据，系统会跳过自定义属性，只应用内建属性和相对矩阵。
...
### Relative 模式不会应用自定义属性

Relative Pose 文件会完整保存 IK/FK、空间切换、手指开关及其他用户属性；但在 **Relative** 勾选状态下，这些自定义属性不会被应用。若需要恢复它们，请取消勾选 **Relative**，按原版 Pose 方式加载。
