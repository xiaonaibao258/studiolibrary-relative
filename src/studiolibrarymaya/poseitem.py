...
class PoseItem(baseitem.BaseItem):
    ...
    def loadSchema(self):
        ...
        :rtype: list[dict]
        """
        hasRelativeTransforms = self.transferObject().hasRelativeTransforms()
        ...
        schema = [
            ...
            },
            {
                ...
                "type": "bool",
                "inline": True,
                # This is pose data, not a user preference: opening a pose
                # must reflect whether that exact file contains matrices.
                "default": hasRelativeTransforms,
                "toolTip": "Apply saved world transforms relative to the pose anchor. "
                           "rootFollow anchors are resolved in the target namespace.",
            },
            {
    ...
    def save(self, objects, **kwargs):
        ...
        mutils.savePose(
            ...
            relativeTransform=kwargs.get("relativeTransform", False),
    ...
    def saveSchema(self):
        ...
        objectsIndex = next(
            (index for index, field in enumerate(schema)
             if field.get("name") == "objects"),
            len(schema),
        )
        schema.insert(objectsIndex, {
            "name": "relativeTransform",
            "title": "Record Relative",
            "type": "bool",
            "default": False,
            "persistent": True,
        ...
        schema.insert(objectsIndex + 1, {
            "name": "useRootFollow",
            "title": "Use Default Relative Object",
            "type": "bool",
            "default": True,
            "persistent": True,
            "toolTip": "Use the configured object in the selected controls' "
                       "namespace as the anchor. When disabled, use the last "
                       "selected object.",
        })
        return schema
